...
public class Users {
	...
	public static boolean UsersScenarioEndToEnd(ExtentReports extent, ExtentTest test){
    	...
    	// prepare test: 
    	// 1. open as NA1 and search for "nusersku" (enrolledvalidated, registered, unenrollment-pending) in users
    	...
    	ArrayList<Boolean> results = new ArrayList<Boolean>(Arrays.asList(prepare, enrollment, unenrollment, registration, valiation, secondEnrollment, update, rolesUpdate, finalUnenrollment));
	...
	public static String usersEnrolUnenrolledUser(ExtentTest test){
    	...
    	ax.result(getDriver(), test, ax.IS_URID(newUserUrid), "USER-GOT-URID "+newUserUrid, "new registered user got URID number");
    	ax.setDriver(2);
    	...
    	ax.result(getDriver(), test, (registr!=null), "USER-ENTER-ENROLLMENT-KEY", "new registered user entered enrollment key");
    	AppSqlQueries.UPDATE_MOBILE_OFF_FOR_USER_URID(newUserUrid);
    	eucr2 						= ax.LoginNA1(2);
    	boolean userStatusAfterEnrolment = Registration_Page.getUserStatusByUrid(getDriver(2), newUserUrid).equals(Registration_Page.USR_STAT_ENROLLED);
	...
	public static boolean usersUnenrolEnrolledUser(ExtentTest test, String newUserUrid){
    	...
    	if(test!=null){
        	ax.result(getDriver(), test, unenrollApproval, "USER-UNENROLLMENT-APPROVED", "new user has approved unenrollment request");
    	}
    	ax.setDriver(3);
    	eucr3 			= ax.LoginNA2(3);
    	boolean userStatusAfterEnrolment = Registration_Page.getUserStatusByUrid(getDriver(3), newUserUrid).equals(Registration_Page.USR_STAT_UNENROLLED);
    	if(test!=null){
	...
	public static String usersValidateUser(ExtentTest test, String newUserUrid){
    	...
    	eucr2.openEucrMenu("Users");
    	String enrollmentKey 		= Registration_Page.registerNewUserApproval(getDriver(2), "", newUserUrid);
    	ax.result(getDriver(), test, ax.IS_URID(newUserUrid), "USER-"+newUserUrid+"-IS-VALIDATED", "user is validated with getting back enrollment key " + enrollmentKey);    	
	...
	public static String usersEnrollUser(ExtentTest test, String newUserUrid, String enrollmentKey){
    	...
    	EucrHomePage eucr2 					= ax.LoginNA1(2);
    	boolean userStatusAfterEnrolment 	= Registration_Page.getUserStatusByUrid(getDriver(2), newUserUrid).equals(Registration_Page.USR_STAT_ENROLLED);
    	ax.result(getDriver(), test, userStatusAfterEnrolment, "USER-VALIDATED-BECOME-ENROLLED", "new user is enrolled");
	...
	public static boolean usersUpdateUserPersonalDetailsProposal(ExtentTest test, String newUserUrid){
		ArrayList<String> valueToUpdate = new ArrayList<String>(Arrays.asList("AxDesign::Corp","AxDesign::HR::Finanse","Head-of-piwnica","Domorowly grzes 4432","Gynsi-Pympek","77890","Lesniewo","SlaskiGor","AT","l@l.pl","l@l.pl","+8777890","+8777890"));
		ax.setDriver(2);
    	EucrHomePage eucr2 			= ax.LoginNA1(2);
    	...
    	ax.result(getDriver(), test, ax.IS_REQUEST_NR(requestNumber), "PROPOSAL-BUSINESS-DETAILS-UPDATE-"+requestNumber, "business details update");
    	
    	EucrHomePage eucr3 = ax.LoginNA2(3);
    	boolean approve = Eucr.ApproveTransactionRequestsEtl(requestNumber);
    	ax.result(getDriver(), test, approve, "APPROVAL-BUSINESS-DETAILS-UPDATE-"+requestNumber, "business details update approval");
    	eucr3 = ax.LoginNA1(2);
    	UsersPoPage users 				=     	(UsersPoPage) eucr3.openEucrMenu("Users");
	...
	public static boolean usersPromoteToSda(ExtentTest test, String newUserUrid){
		...
		ax.result(getDriver(), test, ax.IS_REQUEST_NR(requestNumber), "PROPOSAL-PROMOTE-USER-ROLE-UPDATE-"+requestNumber, "promote user role update");
    	EucrHomePage eucr3 = ax.LoginNA2(3);
    	boolean approve = Eucr.ApproveTransactionRequestsEtl(requestNumber);
